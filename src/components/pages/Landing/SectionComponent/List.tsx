...
const List: React.FC = () => {
  return (
    <ul className="flex overflow-hidden flex-col pb-2 pl-8 mt-4 text-base font-light max-md:pl-5 max-md:max-w-full">
      {listItems.map((item, index) => (
        <li key={index} className={`w-full max-md:max-w-full${index > 0 ? ' mt-2' : ''}`}>
          {item}
        </li>
      ))}
    </ul>
